	...
	if ( parentView ) {
		...
		if ( $.isArray( parentView.data ))  {
			...
			if ( template.slice( 0, 4 ) === "tag:" ) {
				tag = viewsNs.tags[ template.slice( 4 )];
				template = undefined;
			}
			if ( path ) {
...
function createNestedViews( node, parent, nextNode, depth, data, context, prevNode, index ) {
	...
	while ( node && node !== nextNode ) {
		if ( node.nodeType === 1 ) {
		...
		} else if ( node.nodeType === 8 && (tokens = /^(\/?)(?:(item)|(?:(tmpl)(?:\((.*)\))?(?:\s+([^\s]+))?)|(?:\$fn(?:\s+(.+)?)?))$/.exec( node.nodeValue ))) {
			...
			if ( tokens[ 1 ]) {
				...
				if (( tag = currentView.tag ) && tag.model ) {
					currentView.ctx.model = new tag.init( currentView )
				}
				if ( tokens[ 2 ]) {
				...
				if ( tokens[ 2 ]) {
					...
					// A $fn open tag: <!--$fn params-->
					elem = elem2 = node;
					while ( elem2 ) {
						elem2 = elem = elem2.nextSibling
						elem2 = (elem2 && elem2.nodeType !== 1) ? elem2 : undefined;
					};
					if ( elem ) {
						params = /^(\w*)\(([\s\w\[\]\.\$]*)\)(\{.*\}$)?/.exec( tokens[ 6 ]);
						tag = params[ 1 ];
						if ( $.fn[ tag ]) {
							$( elem )[ tag ](Function("return " + params[ 3 ] + ";")());
						}
					}
				}
			}
