...
//===============
		...
		renderTag: function( tagName ) {
			...
			var content, ret, key, view, encoding, hash, l, tagFn, tagLayout,
				json = "",
				path = "",
				keyCount = 0,
				args = slice.call( arguments, 1 ),
				tagObject = viewsNs.tags[ tagName ];
			if ( !tagObject ) {
				return "";
			}
			if ( typeof tagObject === "function" ) {
				tagFn = tagObject;
			} else {
				tagLayout = tagObject.layout;
				tagFn = tagObject.beforeRender;
			}
			...
			ret = tagFn && (tagFn.apply( view, args ) || ""); // If returns false, render empty string

			content = content && ( ret === undefined ) && render( content, view ) || "";

			if ( "" + ret !== ret ) {
				// Not typeof string: No tagFn, or tagFn simply returned true, so render declarative layout/content
				tagLayout = tagLayout && render( tagLayout, view );
				ret = tagLayout
					? content
						? tagLayout.replace( /\{\{>content<\}\}/g, content)
						: tagLayout
					: content;
			}
			if ( tagObject && viewsNs.activeViews ) {
				if ( tagObject.plugin ) {
					ret = "<!--$fn " + tagObject.plugin + "(" + path + ")" + json + "-->" + ret + "<!--/$fn-->";
				}
				if ( tagObject.model ) {
					ret = "<!--tmpl(" + path + (path && json ? "," : "") + json + ")" + " tag:" + tagName + "-->" + ret + "<!--/tmpl-->";
				}
			}
...
//===============
		...
		registerTags: function registerTags( name, tag ) {
			if ( typeof name === "object" ) {
				...
				tag = viewsNs.tags[ name ] = typeof tag === "function" ? { beforeRender: tag } : tag;
				if ( tag.model && viewsNs.activeViews ) {
					tag.init = tag.model.init || new Function();
					tag.init.prototype = tag.model;
					tag.model.constructor = tag.init;
				}
			}
			return this;
		},
